...
//à modifier selon le module RFID utilisé
...
/**
 * @brief initialisation du module RFID RC522
 */
void initRC522(){
    /****************************************
     * si nous n'arrivons pas ) initialiser *
     * les pin de la raspberry              *
     ****************************************/
    if (!bcm2835_init()) {
...
/**
 * @brief écriture sur le registre du PCD
 * @param reg corespond au registre sur le quelle on souhaite écrire
 * @param value la valeur que l'on souhaite écrire
 */
...
void PCD_WriteRegister2(	byte reg,		///< The register to write to. One of the PCD_Register enums.
  ...
  for (byte index = 0; index < count; index++) {
    PCD_WriteRegister1(reg, values[index]);
    }
...
/**
 * @brief lecture sur le registre du PCD
 * @param reg = registre que 'lon souhaite lire
 * @return  nous retourne le contenu du registre
 */
byte PCD_ReadRegister1(byte reg){

    char data[2];
    data[0] = 0x80 | ((reg) & 0x7E); //ordre de lecteur sur la premier case de data
    bcm2835_spi_transfern(data,2); //on récupére le contenu du registre
    return (byte)data[1]; //on retourne la donnée
}
...
void  PCD_Init() {
  ...
  PCD_WriteRegister1(ModeReg, 0x3D);		// Default 0x3F. Set the preset value for the CRC coprocessor for the CalcCRC command to 0x6363 (ISO 14443-3 part 6.2.4)
 ...
 */
void  PCD_Reset() {
  PCD_WriteRegister1(CommandReg, PCD_SoftReset);	// Issue the SoftReset command.
...
byte  PCD_GetAntennaGain() {
  return PCD_ReadRegister1(RFCfgReg) & (0x07<<4);
...
bool  PCD_PerformSelfTest() {
  ...
  PCD_WriteRegister2(FIFODataReg, 25, ZEROES); // write 25 bytes of 00h to FIFO
  PCD_WriteRegister1(CommandReg, PCD_Mem); // transfer to internal buffer

  // 3. Enable self-test
  PCD_WriteRegister1(AutoTestReg, 0x09);

  // 4. Write 00h to FIFO buffer
  PCD_WriteRegister1(FIFODataReg, 0x00);

  // 5. Start self-test by issuing the CalcCRC command
  PCD_WriteRegister1(CommandReg, PCD_CalcCRC);
  ...
  byte result[64];
  PCD_ReadRegister2(FIFODataReg, 64, result, 0);

  // Auto self-test done
  // Reset AutoTestReg register to be 0 again. Required for normal operation.
  PCD_WriteRegister1(AutoTestReg, 0x00);

  // Determine firmware version (see section 9.3.4.8 in spec)
  byte version = PCD_ReadRegister1(VersionReg);
 ...
 *
 */
string  GetStatusCodeName(byte code	///< One of the StatusCode enums.
                              ) {
  switch (code) {
 ...
 *
 */
string  PICC_GetTypeName(byte piccType	///< One of the PICC_Type enums.
                             ) {
  switch (piccType) {
